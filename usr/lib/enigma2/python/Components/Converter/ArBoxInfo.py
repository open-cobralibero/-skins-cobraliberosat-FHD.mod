...
class ArBoxInfo(Poll, Converter, object):
	Boxtype = 0
	CpuInfo = 1	
	HddTemp = 2
	TempInfo = 3
	...
	def __init__(self, type):
		...
		'TimeRestartGui': self.TimeRestartGui}[type]
	...
	def getText(self):
		if self.type == self.Boxtype:
			...
			return (_('Skin: ')) + config.skin.primary_skin.value.split('/')[0]

		elif self.type == self.TimeInfo:
			tz = config.timezone.val.value[4:7]
			if not config.timezone.val.value.startswith('(GMT)'):
				if tz.startswith('+0'):
					return ' +' + tz[2:3]
				elif tz.startswith('-0'):
					return ' -' + tz[2:3]
				else:
					return tz
			else:
			...
			if not config.timezone.val.value.startswith('(GMT)'):
				return (_('Timezone: ')) + config.timezone.val.value[0:15] + _('.')
			else:
				return (_('Timezone: ')) + 'GMT+00:00' + _('.')

		elif self.type == self.TimeRestartGui:
			if os.path.exists('/etc/enigma2/profile'):
				try:
					with open('/etc/enigma2/profile', "r") as q:
						return _('TimeRestartGui: %s') % round(float(q.readlines()[-1].split()[0]), 1) + _(' sec')
				except:
					return _('TimeRestartGui: %s') % 'N/A'
			else:
				return _('TimeRestartGui: %s') % 'N/A'
